...
def train_model(model, train_loader, val_loader, num_epochs=10, lr=1e-4, device="cpu"):
    ...
    counter = 0           # contatore epoche senza miglioramento
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f"{drive_folder}/best_unetpp.pth")
            print("✅ Model saved (best)")
            counter = 0  # resetta contatore
        #else:
        #    counter += 1
        #    print(f"⚠️ Val loss non migliorata da {counter} epoche")
        #    if counter >= patience:
        #        print(f"⏹️ Early stopping dopo {counter} epoche senza miglioramento")
        #        break
...
def main():
    ...
    try:
      ...
      print("✅ Modello migliore salvato finora in 'best_unetpp.pth'")
          
    # Visualizza esempio
    model.load_state_dict(torch.load(f"{drive_folder}/best_unetpp.pth", map_location=device))
